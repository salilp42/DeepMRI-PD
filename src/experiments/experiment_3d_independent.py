...
def run_3d_independent_experiment(config):
    datasets = {
        'PPMI': load_ppmi_data,
    ...
    for dataset_name, load_func in datasets.items():
        ...
        for model_name, model in models.items():
            ...
            for fold, (train_idx, val_idx) in enumerate(skf.split(volumes, labels), 1):
                print(f"\nFold {fold}")

                # Prepare data loaders
                train_loader = DataLoader(list(zip(volumes[train_idx], labels[train_idx])), 
                                          batch_size=config['batch_size'], shuffle=True)
                val_loader = DataLoader(list(zip(volumes[val_idx], labels[val_idx])), 
        ...
        # Save results
        joblib.dump(results, f'results/3d_independent_{dataset_name}.joblib')
